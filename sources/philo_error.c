...
/*   Updated: 2022/09/29 16:00:31 by baroun           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int philo_error(int ac, char **av)
{
	...
	i = 0;
	if (ac < 5 && ac > 6)
		return(philo_error_arg_msg);
	while(av[i])
	{
		if (philo_error_int(av[i]))
			return (printf("Error: argument should be numeric."));
		if (philo_error_signe(av[i]))
			return (printf("Error: argument should be positive."));
		i++;
	}
	return (0);
}
